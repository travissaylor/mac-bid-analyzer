...
describe("analyze", () => {
  ...
  describe("printAnalysisSummary", () => {
    ...
    it("should display AI confidence, reasoning, and comparables", () => {
      const item = {
        lot_id: 12345,
        auction_id: 100,
        lot_number: "42",
        ...
        ebay_sold_count: 10,
        ebay_search_query: "upc:012345678901",
        llm_estimate_low: 40.00,
        llm_estimate_mid: 50.00,
        llm_estimate_high: 60.00,
        llm_provider: "gemini",
        llm_confidence: 82,
        llm_reasoning: "Based on similar widgets selling in the $40-60 range",
        llm_comparables: JSON.stringify([
          { name: "Widget A", estimatedPrice: 45.00 },
          { name: "Widget B", estimatedPrice: 55.00 },
        ]),
        recommended_max_bid: 25.00,
        sales_tax_rate: 0.06,
        location_cost: 0,
        location_tier: "home",
        deal_score: 80.0,
      ...
      try {
        printAnalysisSummary({ item, skipped: false });
      } finally {
        console.log = origLog;
      }

      const output = logs.join("\n");
      expect(output).toContain("AI Confidence: 82/100");
      expect(output).toContain("AI Reasoning: Based on similar widgets");
      expect(output).toContain("Widget A: $45.00");
      expect(output).toContain("Widget B: $55.00");
      expect(output).toContain("Blend:");
      expect(output).toContain("eBay $55.00");
      expect(output).toContain("AI $50.00");
      expect(output).toContain("source: blended");
    });
    ...
    it("should truncate long reasoning", () => {
      const item = {
        ...
        llm_provider: "gemini",
        llm_confidence: 50,
        llm_reasoning: "A".repeat(300),
        llm_comparables: null,
        recommended_max_bid: 20.00,
        sales_tax_rate: 0,
        location_cost: 0,
        location_tier: "home",
        deal_score: 75.0,
      ...
      const reasoningLine = logs.find(l => l.includes("AI Reasoning:"));
      expect(reasoningLine).toBeDefined();
      expect(reasoningLine!).toContain("...");
      // 200 char max for the reasoning value: 197 chars + "..."
      const reasoningValue = reasoningLine!.split("AI Reasoning: ")[1];
      expect(reasoningValue.length).toBe(200);
    });

    it("should show source in max bid for non-blended sources", () => {
      const item = {
        lot_id: 12345,
        auction_id: 100,
        lot_number: "42",
      ...
      const output = logs.join("\n");
      expect(output).toContain("source: ebay");
      expect(output).not.toContain("Blend:");
    });
  });
});
